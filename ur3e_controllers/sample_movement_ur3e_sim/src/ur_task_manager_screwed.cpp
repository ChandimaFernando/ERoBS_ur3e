...
using GoalHandlePickPlaceAct = rclcpp_action::ServerGoalHandle<PickPlaceAct>;
  ...
  explicit URTaskManager(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("ur_action_server_node", options), node_{ std::make_shared<rclcpp::Node>("ur_task_manager", options) }
  {
    ...
    this->action_server_ = rclcpp_action::create_server<PickPlaceAct>(
      ...
      "PickPlaceAct_task",
      std::bind(&URTaskManager::handle_goal, this, _1, _2),
      std::bind(&URTaskManager::handle_cancel, this, _1),
      std::bind(&URTaskManager::handle_accepted, this, _1));

      move_group_interface_ = new MoveGroupInterface(node_, "ur_arm");

  // Create a moveit visula tools related node
  moveit_visual_tools_ =  new moveit_visual_tools::MoveItVisualTools{ node_, "ur_arm", 
  ...
  jmg_ = move_group_interface_->getRobotModel()->getJointModelGroup("ur_arm");
...
int main(int argc, char ** argv)
{
  ...
  auto action_server = std::make_shared<URTaskManager>();
...
URTaskManager::URTaskManager(const rclcpp::NodeOptions& options)
  : node_{ std::make_shared<rclcpp::Node>("ur_task_manager", options) }, Node("ur_task_manager2",options )
{
 
  auto action_server = rclcpp_action::create_server<PickPlaceAct>(
      this->get_node_base_interface(),
      this->get_node_clock_interface(),
      this->get_node_logging_interface(),
      this->get_node_waitables_interface(),
...
rclcpp_action::GoalResponse URTaskManager::handle_goal(
  ...
  std::shared_ptr<const PickPlaceAct::Goal> goal)
{
  RCLCPP_INFO(node_->get_logger(), "Received goal request with sample %s", goal->sample_name.c_str());
  (void)uuid;
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}
...
void URTaskManager::create_nodes(){
  ...
  // mtc_planner_node_->grab_from_side("sample2", 2, 4); //called to a service when commented out
   // mtc_planner_node_->grab_from_top("sample1", 0 , 1); // Go to rest location
